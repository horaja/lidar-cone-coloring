...
# ===================================================================
# 2. PyTorch Dataset from .npy Files
# ===================================================================
...
def npy_loader(path):
  """Loads a .npy file and adds a channel dimension for the CNN."""
  sample = np.load(path)
  # Ensure the data type is float32 for the model
  return torch.from_numpy(sample).float().unsqueeze(0)

class NpyConeDataset(DatasetFolder):
  """
  A dataset class for loading .npy files from a directory structure
  where subdirectories are the class names (e.g., data/blue, data/yellow).
  """
...
if __name__ == '__main__':
  # --- Argument Parsing ---
  parser = argparse.ArgumentParser(description='Train a CNN on real LiDAR cone data.')
  ...
  parser.add_argument('--model_save_path', type=str, default='models/cone_classifier_real.pth', help='Path to save the trained model.')
  ...
  full_dataset = NpyConeDataset(root=args.data_dir)
  ...
  for epoch in range(args.epochs):
    ...
    print(f'Epoch [{epoch + 1}/{args.epochs}], Train Loss: {epoch_loss:.4f}, Val Loss: {val_loss / len(val_loader):.4f}, Val Accuracy: {val_accuracy:.2f}%')
